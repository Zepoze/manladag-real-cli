...
import { updateDataJSON }  from './updateLibsData'
...
async function go() {
  ...
  if(ListrTasks) new Listr([{ title: 'Libs '+(answer1.action ==1 ? 'Installation': (answer1.action ==2 ? 'Uninstallation': 'Update')),task: ()=> new Listr(ListrTasks, { concurrent: true })}, {title: 'Save files data', task: () => updateDataJSON(manager)}]).run()
